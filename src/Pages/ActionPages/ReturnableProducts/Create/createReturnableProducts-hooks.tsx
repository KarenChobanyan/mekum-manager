...
const useCreateReturnableProducts = () => {
    ...
    const onSubmit: SubmitHandler<IReturnableProductsForm> = (values) => {
        const payload: PostRetunableRequestData = values.products?.map((item): IPostReturnable => {
            return {
                material_value_id_in: +(item.material_value_id_in?.id!),
                material_value_id_out: +(item.material_value_id_out?.id!)
            };
        });
        add(payload)
    };
...
export default useCreateReturnableProducts